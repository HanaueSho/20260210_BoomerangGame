...
void ResultSpriteObject::Init()
{
    // 1) Transform（既に GameObject ctor で追加済み）を取得して初期姿勢を入れておく
    auto* tf = GetComponent<TransformComponent>();
    tf->SetPosition({ SCREEN_WIDTH / 2.0f, SCREEN_HEIGHT / 2.0f - 200.0f, 0 });
    tf->SetScale({ 1,1,1 });
    ...
    ID3D11ShaderResourceView* srv = Texture::LoadAndRegisterKey("assets\\texture\\gameSuccess.png");
...
void ResultSpriteObject::Update(float gameDt, float realDt)
{
...
void ResultSpriteObject::SetType(Type type)
{
    m_Type = type;
    ...
    if (m_Type == Type::Success)
    {
        ...
        MeshFactory::CreateQuad3D(mf, { 200.0f, 50.0f, true });
    }
